...
class NeuralNetwork():
	...
	def GenerateRandomDirection(self, angle, snake_pos):
		...
		if angle > 0:
			...
			direction = 0
		return self.FindNextDirection(angle, snake_pos, direction)

	# Finds the next calculated direction
	def FindNextDirection(self, angle, snake_pos, direction):
	...
	def GenerateOutput(self, inputs, training_data):
		...
		if direction == goLeft:
			if isLeftBlocked:
				if isFrontBlocked and not isRightBlocked:
					direction, button = self.FindNextDirection(angle, snake_pos, goRight)
					output = self.goRight
				elif not isFrontBlocked and isRightBlocked:
					direction, button = self.FindNextDirection(angle, snake_pos, goFront)
					output = self.goFront
				elif not isFrontBlocked and not isRightBlocked:
					direction, button = self.FindNextDirection(angle, snake_pos, goFront)
					output = self.goFront
			else:
				direction, button = self.FindNextDirection(angle, snake_pos, goLeft)
				output = self.goLeft

		elif direction == goFront:
			if isFrontBlocked:
		...
		return training_data, direction, button
	...
	def GenerateData(self):
		...
		for _ in tqdm(range(training_games)):
			...
			for _ in range(steps_per_game):
				...
				direction, button = self.GenerateRandomDirection(angle, snake_pos)
				training_data_y, direction, button = self.GenerateOutput([isLeftBlocked, isFrontBlocked, isRightBlocked, angle, snake_pos, direction], training_data_y)
